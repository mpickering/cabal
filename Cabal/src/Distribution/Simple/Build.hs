...
module Distribution.Simple.Build
  ( build, build_setupHooks
  , repl, repl_setupHooks
  , startInterpreter
...
build_setupHooks
  ...
  pkg_descr lbi flags suffixes = do
  ...
  (\f -> foldM_ f (installedPkgs lbi) componentsToBuild) $ \index target -> do
    preBuildComponent
      (fmap ($ BuildNormal flags) preBuildComponentHook)
    ...
    mb_ipi <-
      buildComponent
        ...
        clbi
        distPref
    for_ postBuildComponentHook $ \ postBuild_hook ->
      postBuild_hook (BuildNormal flags) lbi' target
...
repl_setupHooks
  (BuildHooks { preBuildComponentHook, postBuildComponentHook })
  pkg_descr lbi flags suffixes args = do
  ...
  let lbiForComponent comp lbi' =
      ...
      preBuildHook = fmap ($ BuildRepl flags) preBuildComponentHook
  ...
  sequence_
    [ do
      ...
      preBuildComponent preBuildHook verbosity lbi subtarget
      buildComponent
        verbosity
        NoFlag
        pkg_descr
  ...
  preBuildComponent preBuildHook verbosity lbi target
  replComponent replFlags verbosity pkg_descr lbi' suffixes comp clbi distPref
  for_ postBuildComponentHook $ \ postBuild -> do
    postBuild (BuildRepl flags) lbi target
...
preBuildComponent
  :: Maybe (LocalBuildInfo -> TargetInfo -> IO ())
  -- ^ pre-build hook
  -> Verbosity
  -> LocalBuildInfo
  -- ^ Configuration information
  -> TargetInfo
  -> IO ()
preBuildComponent mb_preBuildHook verbosity lbi tgt = do
