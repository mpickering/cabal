...
data PackageInfo = PackageInfo
  ...
  , promisedDepsSet :: Map (PackageName, ComponentName) (Version, ComponentId)
...
mkPromisedDepsSet :: [PromisedComponent] -> Map (PackageName, ComponentName) (Version, ComponentId)
mkPromisedDepsSet comps = Map.fromList [((pn, CLibName ln), (v, cid)) | PromisedComponent pn ln v cid <- comps]
  ...
  (Dependency dep_pkgname vr libs) =
    ...
    where
      ...
      do_external_external lib
        | Just (ver, cid) <- Map.lookup (dep_pkgname, CLibName lib) promisedIndex =
            return $ PromisedDependency (ConfiguredPromisedComponent dep_pkgname (AnnotatedId (PackageIdentifier dep_pkgname ver) (CLibName lib) cid))
      ...
      do_external_internal lib
        | Just (pv, cid) <- Map.lookup (dep_pkgname, CLibName lib) promisedIndex =
            return $ PromisedDependency (ConfiguredPromisedComponent dep_pkgname (AnnotatedId (PackageIdentifier dep_pkgname pv) (CLibName lib) cid))
