...
buildOrReplLib mReplFlags verbosity numJobs pkg_descr lbi lib clbi = do
  let uid = componentUnitId clbi
      ...
      whenReplLib k = forM_ mReplFlags k
  ...
  print ("REPL?", unUnitId uid, forRepl)
  ...
  whenReplLib $ \rflags -> do
    ...
    writeMultfileOrRepl runGhcProg comp platform replOpts pkg_descr clbi rflags
...
writeMultfileOrRepl :: (GhcOptions -> IO ()) -> (Compiler) -> (Platform) -> (GhcOptions) -> PackageDescription -> (ComponentLocalBuildInfo) -> ReplOptions -> IO ()
writeMultfileOrRepl runGhcProg comp platform replOpts pkg_descr clbi rflags = do
    case replOptionsMultiFile rflags  of
      Flag outfile -> do
        srcdir <- getCurrentDirectory
        putStrLn ("WRITING:" ++ outfile)
        putStrLn srcdir
        let reexported_modules = [mn | LibComponentLocalBuildInfo {} <- [clbi], IPI.ExposedModule mn (Just {}) <- componentExposedModules clbi]
        let extra_opts = ["-this-package-name", prettyShow (pkgName (PD.package pkg_descr))
                         ,"-working-dir", srcdir]
                         ++ (concatMap (\m -> ["-reexported-module", prettyShow m]) reexported_modules)
            removeMake ("--make":xs) = xs
            removeMake ("--interactive":xs) = xs
            removeMake (x:xs) = x: removeMake xs
            removeMake [] = []
            addExtraOpts opts = removeMake opts ++ extra_opts
        let opts = addExtraOpts $ renderGhcOptions comp platform replOpts
        writeFile outfile $ unlines opts
