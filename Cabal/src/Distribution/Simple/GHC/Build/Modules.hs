...
buildHaskellModules numJobs ghcProg pkg_descr buildTargetDir wantedWays pbci = do
  ...
  (symbolicInputFiles, inputModules) <- componentInputs buildTargetDir pkg_descr pbci
  let inputFiles = map getSymbolicPath symbolicInputFiles
  ...
  let
    ...
    baseOpts way =
      (Internal.componentGhcOptions verbosity lbi bi clbi buildTargetDir)
        `mappend` mempty
          ...
          , ghcOptHPCDir = fmap i $ hpcdir (buildWayHpcWay way) -- maybe this should not be passed for vanilla?
          }
      where
    ...
    dynTooOpts =
      (baseOpts StaticWay)
        ...
        , ghcOptHPCDir = fmap i $ hpcdir Hpc.Dyn
