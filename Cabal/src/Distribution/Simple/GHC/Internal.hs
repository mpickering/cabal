...
componentCcGhcOptions verbosity lbi bi clbi odir filename =
  mempty
    ...
    , ghcOptInputFiles = toNubListR [u filename]
...
componentGhcOptions verbosity lbi bi clbi odir =
  let implInfo = getImplInfo $ compiler lbi
   in mempty
        ...
        , ghcOptObjDir = toFlag $ u odir
        , ghcOptHiDir = toFlag $ u odir
        , ghcOptHieDir = bool NoFlag (toFlag $ u $ coerceSymbolicPath odir </> (extraCompilationArtifacts </> makeRelativePathEx "hie")) $ flagHie implInfo
        , ghcOptStubDir = toFlag $ u odir
        , ghcOptOutputDir = toFlag $ u odir
  ...
  where
    ...
    exe_paths =
      [ u $ componentBuildDir lbi (targetCLBI exe_tgt)
