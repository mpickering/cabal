...
buildOrReplLib mReplFlags verbosity numJobs _pkg_descr lbi lib clbi = do
  ...
  let cLikeFiles = fromNubListR $ toNubListR (cSources libBi) <> toNubListR (cxxSources libBi)
      jsSrcs = map u $ jsSources libBi
      cObjs = map ((`replaceExtension` objExtension) . u) cLikeFiles
      ...
      vanillaOptsNoJsLib =
        baseOpts
          `mappend` mempty
            ...
            , ghcOptHPCDir = fmap u $ hpcdir Hpc.Vanilla
            }
      ...
      profOpts =
        adjustExts "p_hi" "p_o" vanillaOpts
          `mappend` mempty
            ...
            , ghcOptHPCDir = fmap u $ hpcdir Hpc.Prof
            }
      ...
      sharedOpts =
        adjustExts "dyn_hi" "dyn_o" vanillaOpts
          `mappend` mempty
            ...
            , ghcOptHPCDir = fmap u $ hpcdir Hpc.Dyn
            }
  ...
  when has_code . when False {- fixme nativeToo -} . unless forRepl $ do
    ...
    let cSharedObjs =
          map
            ((`replaceExtension` ("dyn_" ++ objExtension)) . u)
            (cSources libBi ++ cxxSources libBi)
        compiler_id = compilerId (compiler lbi)
        sharedLibFilePath = u libTargetDir </> mkSharedLibName (hostPlatform lbi) compiler_id uid
        staticLibFilePath = u libTargetDir </> mkStaticLibName (hostPlatform lbi) compiler_id uid
    ...
    unless (null hObjs && null cObjs && null stubObjs) $ do
      ...
      let staticObjectFiles =
            hObjs
              ++ map (u libTargetDir </>) cObjs
              ++ stubObjs
          dynamicObjectFiles =
            hSharedObjs
              ++ map (u libTargetDir </>) cSharedObjs
              ++ stubSharedObjs
          ...
          ghcSharedLinkArgs =
            mempty
              ...
              , ghcOptLinkLibPath = toNubListR $ map u $ extraLibDirs libBi
    ...
    exeSources exe@Executable{buildInfo = bnfo, modulePath = modPath} = do
      ...
      if haskellMain || pkgId == fakePackageId
        then
          ...
          let (csf, cxxsf)
                | isCxx (u main) = (cSources bnfo, main : cxxSources bnfo)
...
gbuild verbosity numJobs pkg_descr lbi bm clbi = do
  ...
  let isCoverageEnabled = exeCoverage lbi
      hpcdir way
        ...
        | isCoverageEnabled = toFlag $ u $ Hpc.mixDir (tmpDir </> coerceSymbolicPath extraCompilationArtifacts) way
        | otherwise = mempty
  ...
  let cSrcs = cSourcesFiles buildSources
      ...
      dynamicTooSupported = supportsDynamicToo comp
      cObjs = map ((`replaceExtension` objExtension) . u) cSrcs
      cxxObjs = map ((`replaceExtension` objExtension) . u) cxxSrcs
            ...
            , ghcOptInputFiles =
                toNubListR $
                  if package pkg_descr == fakePackageId
                    then filter (isHaskell) $ map u inputFiles
                    else map u inputFiles
            , ghcOptInputScripts =
                toNubListR $
                  if package pkg_descr == fakePackageId
                    then filter (not . isHaskell) $ map u inputFiles
                    else []
      ...
      linkerOpts =
        mempty
          ...
          , ghcOptLinkLibPath = toNubListR $ map u $ extraLibDirs bnfo
  ...
  unless (null cxxSrcs) $ do
    ...
    sequence_
      [ do
        ...
        createDirectoryIfMissingVerbose verbosity True odir
        needsRecomp <- checkNeedsRecompilation (i filename) opts
        when needsRecomp $
          runGhcProg opts
      | filename <- cxxSrcs
      ]
  ...
  case bm of
    ...
    GBuildExe _ -> do
      ...
      runGhcProg linkOpts{ghcOptOutputFile = toFlag targetPath}
    GBuildFLib flib -> do
      ...
      runGhcProg linkOpts{ghcOptOutputFile = toFlag $ u buildFile}
