    ...
    , runPreProcessor = mkSimplePreProcessor $ \inFile outFile verbosity -> do
        ...
        runProgramCwd verbosity (mbWorkDirLBI lbi) prog $
          ["-E", "-cpp"]
            ...
            ++ ["-optP-include", "-optP" ++ getSymbolicPath (autogenComponentModulesDir lbi clbi </> makeRelativePathEx cppHeaderName)]
            ++ ["-o", outFile, inFile]
            ++ extraArgs
    }
        ...
        runProgramCwd verbosity (mbWorkDirLBI lbi) cpphsProg $
          ("-O" ++ outFile)
            ...
            : ( if cpphsVersion >= mkVersion [1, 6]
                  then ["--include=" ++ getSymbolicPath (autogenComponentModulesDir lbi clbi </> makeRelativePathEx cppHeaderName)]
                  else []
              )
            ++ extraArgs
    }
        ...
        if hsc2hsSupportsResponseFiles
          then
            withResponseFile
              ...
              pureArgs
              ( \responseFileName ->
                  runHsc2hs (prependCrossFlags ["@" ++ u responseFileName])
              )
  ...
  where
    ...
    genPureArgs hsc2hsVersion gccProg inFile outFile =
      ...
      | opt <-
        ...
        ++ ["--cflag=-I" ++ getSymbolicPath dir | dir <- PD.includeDirs bi]
        ++ [ "--cflag=-I" ++ getSymbolicPath (buildDir lbi </> unsafeCoerceSymbolicPath relDir)
        ...
        ++ [ "--cflag=" ++ opt
           | opt <-
              [ "-I" ++ getSymbolicPath (autogenComponentModulesDir lbi clbi)
              , "-I" ++ getSymbolicPath (autogenPackageModulesDir lbi)
              , "-include"
              , getSymbolicPath $ autogenComponentModulesDir lbi clbi </> makeRelativePathEx cppHeaderName
              ]
           ]
        ++ [ "--lflag=-L" ++ u opt
           | opt <-
         ...
         verbosity -> do
            ...
            runProgramCwd verbosity mbWorkDir c2hsProg $
              ...
              ["--cpp=" ++ programPath gccProg, "--cppopts=-E"]
                ...
                ++ ["--cppopts=-include" ++ getSymbolicPath (autogenComponentModulesDir lbi clbi </> makeRelativePathEx cppHeaderName)]
                ++ ["--include=" ++ outBaseDir]
                -- Options from dependent packages
                ++ [ "--cppopts=" ++ opt
                   | pkg <- pkgs
