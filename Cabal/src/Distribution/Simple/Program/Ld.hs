...
combineObjectFiles verbosity lbi ldProg target files = do
  ...
  let
    ...
    invokeWithResponseFile :: SymbolicPath "Package" (File "Response") -> ProgramInvocation
    invokeWithResponseFile atFile =
      ld $ simpleArgs ++ ['@' : u atFile]
  ...
  if oldVersionManualOverride || responseArgumentsNotSupported
    ...
    else withResponseFile verbosity defaultTempFileOptions mbWorkDir targetDir "ld.rsp" Nothing files $
