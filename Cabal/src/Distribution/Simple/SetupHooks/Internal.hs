            ...
            Nothing -> do
              case cmds of
                StaticRuleCommand{staticRuleCommand = cmd} ->
                  runCommand cmd
                DynamicRuleCommands{dynamicRuleCommand = cmd}
                  | Just (_dynDeps, dynRes) <- mbDyn ->
                      runCommand cmd (unsafeCoerce dynRes)
                  | otherwise ->
                      error $ "internal error: no dynamic dependency result data for " ++ show rId
...
resolveDependency :: Verbosity -> RuleId -> Map RuleId Rule -> Rule.Dependency -> IO Location
