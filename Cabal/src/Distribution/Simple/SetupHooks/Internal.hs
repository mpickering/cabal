...
executeRules verbosity lbi tgtInfo rulesFromInputs inputs = do
  ...
  let
    ...
    getRuleDependencies (Rule{dependencies = deps}) =
      nub $
        concat $
          [ mapMaybe
            (\(rId, r) -> if r `ruleOutputsLocation` dep then Just rId else Nothing)
            allRules
          | dep <- deps
          ]
    ...
    [] -> do
        ...
        warn verbosity $
          unlines $
            "The following rules are not demanded and will not be run:"
              : [ "  - Rule " ++ show rId ++ ", generating " ++ showLocs (NE.toList $ results r)
      ...
      for_ sccs $ \(Graph.Node ruleVertex _) ->
        ...
        unless (ruleVertex `Set.member` nonDemandedRuleVerts) $ do
          let (r@Rule{actionId = actId, dependencies = deps, results = reslts}, ruleId, _) =
