...
data LocalBuildInfo =
  NewLocalBuildInfo
    ...
    , localBuildConfig :: LocalBuildConfig
      -- ^ Information about a package that can be configured by the user.
    }
...
buildDir lbi =
  buildDirPBD $ packageBuildDescr $ localBuildDescr lbi

buildDirPBD :: PackageBuildDescr -> FilePath
buildDirPBD (PackageBuildDescr { configFlags = cfg })
