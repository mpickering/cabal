...
import Distribution.Types.LocalBuildInfo ( LocalBuildInfo, withPrograms )
...
withSetupMethod verbosity options pkg buildType' allowInLibrary with
  ...
  -- SetupHooks TODO: the following guard should be lower, but I want to test
  -- the InLibrary method.
  | AllowInLibrary <- allowInLibrary
  = do
    -- SetupHooks TODO: getExternalSetupMethod compiles the hooks executable.
    -- That functionality should be moved here.
    when (buildType' == Hooks) $
      void $ getExternalSetupMethod verbosity options pkg Hooks
  ...
  | otherwise = return $ with (cabalVersion, InternalMethod, options)
        ...
        InLibraryArgs libArgs ->
          case libArgs of
            InLibraryConfigureArgs elabSharedConfig elabReadyPkg -> do
              let progs0 = Cabal.addKnownPrograms Cabal.builtinPrograms $ pkgConfigCompilerProgs elabSharedConfig
              putStrLn $ unlines
                [ "About to call in-library configure"
                , "unconfigured progs: " ++ show (Cabal.unconfiguredProgs progs0)
                , "configured progs: " ++ show (Map.keys $ Cabal.configuredProgs progs0) ]
              lbi <-
                InLibrary.configure
                  (InLibrary.libraryConfigureInputsFromElabPackage elabSharedConfig elabReadyPkg)
                  flags
              putStrLn $ unlines
                [ "Finished calling in-library configure"
            ...
            InLibraryPostConfigureArgs sPhase mbLBI ->
              case mbLBI of
                NotInLibraryNoLBI ->
                  error $ unlines
                    [ "SLD made a bug: in-library post-conf but no LBI"
                    , "callStack: " ++ prettyCallStack callStack ]
                  -- TODO: I think this happens when we need to re-build a package
                  -- but not re-configure it. Then we get a NotInLibraryNoLBI
                  -- instead of a InLibraryLBI.
                  --
                  -- See "whenReconfigure" in Distribution.Client.ProjectBuilding.UnpackedPackage.
                InLibraryLBI lbi ->
                  case sPhase of
                    SBuildPhase    -> InLibrary.build    flags lbi
                    SHaddockPhase  -> InLibrary.haddock  flags lbi
                    SReplPhase     -> InLibrary.repl     flags lbi
                    SCopyPhase     -> InLibrary.copy     flags lbi
                    STestPhase     -> InLibrary.test     flags lbi
                    SBenchPhase    -> InLibrary.bench    flags lbi
                    SRegisterPhase -> InLibrary.register flags lbi
        NotInLibrary -> error "internal bug: NotInLibrary argument but getSetup chose InLibrary"
...
getExternalSetupMethod verbosity options pkg bt = do
  ...
  where
    ...
    i :: SymbolicPathX allowAbs "Package" to -> FilePath
    ...
    compileHooksExecutable opts ver mbCompId forceCompile
      = compileSetupExecutableX "SetupHooks" hooksHs hooksProgFile opts ver mbCompId forceCompile
    ...
    compileSetupExecutableX
      :: String
      -> SymbolicPath "Package" (File inFile)
      -> SymbolicPath "Package" (File outFile)
      -> SetupScriptOptions
      -> Version
      -> Maybe ComponentId
      -> Bool
    ...
    compileSetupExecutableX
      ...
      maybeCabalLibInstalledPkgId
      forceCompile = do
        setupXHsNewer <- i inPath `moreRecentFile` i outPath
        ...
        when (outOfDate || forceCompile) $ do
          ...
          let cabalPkgid = PackageIdentifier (mkPackageName "Cabal") cabalLibVersion
              ...
              -- opted-in and declared a custom-setup stanza.
              --
              selectedDeps
                ...
                -- SetupHooks TODO: when compiling the external HooksHs,
                -- we need to add a dependency on the hooks-exe library.
                -- However, we might not have installed that library yet... hmm...
              addRenaming (ipid, _) =
              ...
              ghcOptions =
                mempty
                  ...
                  , ghcOptInputFiles = toNubListR [u inPath]
                  , ghcOptOutputFile = Flag $ u outPath
                  , ghcOptObjDir = Flag $ u setupDir
                  , ghcOptHiDir = Flag $ u setupDir
          ...
          putStrLn $ unlines
            [ "Compiling " ++ what
            , "selectedDeps = " ++ show selectedDeps
            , "ghcCmdLine = " ++ show ghcCmdLine ]
